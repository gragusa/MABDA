...
library(caret)  ## this package is used for confusionMatrix which 
                ## gives information about the predicitive performances of the model
...
dtm <- model.matrix(~income+age+race+sex+education, data = dt_all)
...
## procValue
procValues    <- preProcess(X_train[idx_continuous,-1], method = c("center", "scale"))
...
## you should try to increase the number of regressors, in particular adding nonlinear term, 
## e.g. I(age^2), I(age^4)) and adding other continuous variables.
glm_1 <- glm.fit(X_scaledTrain, y_train, family = binomial(link = "logit"))

fitted_glm <- predict_logit(X_scaledTrain, coef(glm_1))

## This should converge in one iteration, since the glm coefficient set the gradient == 0
coeff <- gradd_logit(y_train, X_scaledTrain, a = 0.9, 
...
coeff_reg <- gradd_logit(y_train, X_scaledTrain, a = .9, 
                     theta0 = coef(glm_1), debug = TRUE, 
                     epsilon = 1e-10, max_iter = 20000, lambda = 0.01)

## Make FITTED with the regularized model
fitted_reg <- predict_logit(X_scaledTrain, coeff_reg)
...
predict_reg <- predict_logit(X_scaledTest, coeff_reg)
predict_glm <- predict_logit(X_scaledTest, coef(glm_1))
...
out <- nnet(X_scaledTrain, y_train, size = 14, entropy = TRUE, maxit = 1000)
plotnet(out)
nn_pred <- ifelse(predict(out, newdata = X_scaledTest)>0.5, 1, 0)
...
nn1 <- neuralnet(formula, data = cbind(income= y_train, X_scaledTrain), 
                 hidden = 14, err.fct = "ce", 
                 lifesign = "full", linear.output = FALSE, startweights = array(0, 1200))

## plot(nn1)  this will plot the graphic representation of the neural net
plotnet(nn1)
...
fitted_nn <- neuralnet::compute(nn1, X_scaledTest[,-1])
fitted_nnet <- ifelse(fitted_nn$net.result>0.5, 1, 0)
confusionMatrix(fitted_nnet, y_test)
